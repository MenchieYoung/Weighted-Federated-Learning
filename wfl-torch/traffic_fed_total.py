# -*- coding: utf-8 -*-
"""
Created on Wed Dec 11 10:38:30 2019
...
from models.Update_mod import LocalUpdate_traffic_mod
from models.Nets import trafficpred
from models.Fed import FedDyUp
...
#            lc_res=[] 
#            lc_res=np.array(lc_res)
            local = LocalUpdate_traffic_mod(train_dataset=Traffic(datasets[idx]),test_dataset=Traffic(testsets[idx]), idxs=dict_users[idx])
#            print(type(Traffic(datasets[idx])))
            w, loss,res = local.train(net=copy.deepcopy(net_glob))
            lc_res=res
            tmp_res.append((lc_res-lctmp_res)+1)
