...
from models.Update_mod import LocalUpdate
...
#    dict_users=unbalanced_dataset(USER)
    ...
    for iter in range(EPOCH):
        ...
        for idx in range(idxs_users):
            ...
            lc_res=[] 
            lc_res=np.array(lc_res)
            local = LocalUpdate( train_dataset=Vehicle_train(datasets[idx]),test_dataset=Vehicle_test(testsets[idx]), idxs=dict_users[idx])
